...
pub fn load_font_default() {
    unsafe {
        if DEFAULT_FONT.glyph_count > 0 { return; }
        ...
        let chars_width: [i32; 224] = [ 3, 1, 4, 6, 5, 7, 6, 2, 3, 3, 5, 5, 2, 4, 1, 7, 5, 2, 5, 5, 5, 5, 5, 5, 5, 5, 1, 1, 3, 4, 3, 6,
        ...
        DEFAULT_FONT.recs = libc::malloc(std::mem::size_of::<Rectangle>() * 224) as *mut Rectangle;
        DEFAULT_FONT.glyphs = libc::malloc(std::mem::size_of::<GlyphInfo>() * 224) as *mut GlyphInfo;
        ...
        for i in 0..224 {
            ...
            if test_pos_x >= 128 {
                ...
                *DEFAULT_FONT.recs.add(i) = updated_rec;
                current_pos_x = test_pos_x;
            } else {
                *DEFAULT_FONT.recs.add(i) = rec;
                current_pos_x = test_pos_x;
            }
...
pub fn draw_text(text: &str, x: i32, y: i32, font_size: i32, color: Color) {
    ...
    draw_text_ex(&font, text, Vector2::new(x as f32, y as f32), font_size as f32, 2.0, color);
}
