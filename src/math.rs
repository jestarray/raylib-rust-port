...
pub fn Vector3Unproject(source: Vector3, projection: Matrix, view: Matrix) -> Vector3 {
    // The former Matrix::multiply(view, projection) operation evaluated to
    // projection * view because raymath's multiplication argument order is
    // reversed relative to glam's `Mul` implementation.
    let transformed = (projection * view).inverse() * source.extend(1.0);
