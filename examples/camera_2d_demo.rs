...
use raylib_rs::rtext::*;
use raylib_rs::types::{Color, Rectangle, Vector2, Camera2D};
...
fn main() {
    ...
    init_window(screen_width, screen_height, "raylib-rs [core] example - 2d camera");
    ...
    while !window_should_close() {
        // Update
        if is_key_down(KEY_RIGHT) { player.x += 2.0; }
        else if is_key_down(KEY_LEFT) { player.x -= 2.0; }

        camera.target = Vector2::new(player.x + 20.0, player.y + 20.0);

        if is_key_down(KEY_A) { camera.rotation -= 1.0; }
        else if is_key_down(KEY_S) { camera.rotation += 1.0; }

        if camera.rotation > 40.0 { camera.rotation = 40.0; }
        else if camera.rotation < -40.0 { camera.rotation = -40.0; }
        ...
        begin_mode_2d(camera);
            ...
            draw_line(camera.target.x as i32, -screen_height * 10, camera.target.x as i32, screen_height * 10, Color::GREEN);
            draw_line(-screen_width * 10, camera.target.y as i32, screen_width * 10, camera.target.y as i32, Color::GREEN);
        end_mode_2d();
